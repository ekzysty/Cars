        ...
        marker:{
            options: {
                icon: new google.maps.MarkerImage(
                    path+"assets/img/mapmarker.png",
                    new google.maps.Size(59, 58, "px", "px"),
                    new google.maps.Point(51.503454,-0.119562),    //sets the origin point of the icon
    ...
    dataType: "json",
      complete: ({status}) => {
        ...
        if (status !== 200) {
            ...
            $('#contact-form-msg').text('Что-то пошло не так');
            $('#contact-form input[type=submit]').removeAttr('disabled');
        }
      },
  });
...
$( "#car-select-form" ).submit(function() {
  ...
  var selectedCarImage = $("#car-select").find(":selected").data('carImage');
  ...
  if(0 === error)
  {
    ...
    $("#selected-car").val(selectedCarVal);
    ...
    $("#pick-up").val(pickUpDate + ' ' + pickUpTime);

    $("#drop-off-date-ph").html(dropOffDate);
    $("#drop-off-time-ph").html(dropOffTime);
    $("#drop-off").val(dropOffDate + ' ' + dropOffTime);
